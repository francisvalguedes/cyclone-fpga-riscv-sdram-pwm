...
`timescale 1 ps / 1 ps
module niosV_ip_intel_generic_serial_flash_interface_top_0_xip_controller #(
...
// enabling read or write operation
    ...
    // mem_byteenable might be all zeros for mem_burstcount > 1 
    assign mem_wr_combi = mem_wr && ~mem_waitrequest && ((mem_burstcount > 7'b1) || (mem_byteenable != 4'h0));
...
// *********************************************************************************
    ...
    always_comb begin
        case (mem_byteenable_reg)
            ...
            4'b0111: begin
                fifo_in_data_wire = {8'b0, mem_write_data_reg[23:0]};
                addr_bytes_xip   = {mem_addr_reg[ADDR_WIDTH-3:0], 2'b0}; 
            end

            4'b1110: begin
                fifo_in_data_wire = {8'b0, mem_write_data_reg[31:8]};
            ...
            4'b0110: begin
                fifo_in_data_wire = {16'b0, mem_write_data_reg[23:8]};
                addr_bytes_xip   = {mem_addr_reg[ADDR_WIDTH-3:0], 2'b0} + 32'h1; 
            end
    ...
    // the data is direct from avalon write data
    assign fifo_in_data[ 7: 0] = (mem_byteenable[0] == 1'b1) ? mem_wrdata[ 7: 0] : 8'hFF;
    assign fifo_in_data[15: 8] = (mem_byteenable[1] == 1'b1) ? mem_wrdata[15: 8] : 8'hFF;
    assign fifo_in_data[23:16] = (mem_byteenable[2] == 1'b1) ? mem_wrdata[23:16] : 8'hFF;
    assign fifo_in_data[31:24] = (mem_byteenable[3] == 1'b1) ? mem_wrdata[31:24] : 8'hFF;
